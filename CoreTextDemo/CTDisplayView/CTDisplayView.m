...
@property (nonatomic) NSInteger selectionStartPosition; // 停泊点起始位置
@property (nonatomic) NSInteger selectionEndPosition; // 停泊点终止位置
...
- (UIImage *)cursorWithFontHeight:(CGFloat)height isTop:(BOOL)top {
    ...
    UIGraphicsBeginImageContext(rect.size); // 开始 image 绘制
    CGContextRef context = UIGraphicsGetCurrentContext();
    // draw point
    if (top) {
        CGContextAddEllipseInRect(context, CGRectMake(0, 0, 22, 22)); // 绘制圆
    } else {
...
- (void)userLongPressedGuestureDetected:(UILongPressGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan ||
        ...
        if (index != -1 && index < self.data.content.length) {
            ...
            _selectionEndPosition = index + 2; // 选中的字体个数为：2
        }
 ...
 * 3. 在显示菜单前，必须调用：
     [self becomeFirstResponder]，成为第一个响应者
...
#pragma mark - UIMenuController BindingEvents
- (void)cut:(id)sender
{
